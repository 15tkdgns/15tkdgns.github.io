...
class DashboardExtensions {
    ...
    init() {
        console.log('DashboardExtensions init() started');
    ...
    renderLocalXaiAnalysis(stockSymbol) {
        console.log(`Rendering individual XAI analysis: ${stockSymbol}`);

        // SHAP Force Plot (simulated with HTML)
        ...
        if (forcePlotContainer) {
            ...
            const positiveFeatures = [{name: '5-day Volume', value: 3.2}, {name: 'News Sentiment', value: 2.8}];
            const negativeFeatures = [{name: '20-day MA', value: -0.8}];
        ...
        // LIME Explanation (simulated with HTML)
        ...
        if (limeContainer) {
            limeContainer.innerHTML = `
                ...
                <ul class="lime-explanation-list">
                    <li class="positive"><strong>News: "New Product Launch"</strong> contributed <strong>+3.5%</strong> to the prediction</li>
                    <li class="positive"><strong>10% Increase in Volume</strong> contributed <strong>+1.8%</strong> to the prediction</li>
                    <li class="negative"><strong>RSI exceeding 75</strong> contributed <strong>-2.1%</strong> to the prediction</li>
                </ul>
                <p class="xai-explanation">
                    LIME shows the top 3-5 factors that had the greatest impact on a specific prediction.
                </p>
            `;
        }
    ...
    async loadXAIData() {
        try {
            console.log('Attempting to load XAI data...');
            ...
            if (data && data.explainability) {
                this.xaiData = data;
                console.log('XAI data loaded successfully:', data);
                
                // Update charts if XAI data is successfully loaded
                console.log('Starting chart update with actual XAI data');
                this.updateXAICharts();
            } else {
                console.warn('XAI data is not in the correct format. Using mock data.');
                this.xaiData = this.generateMockXAIData();
                this.updateXAICharts();
            }
        } catch (error) {
            console.error('XAI data load failed:', error);
            console.log('Falling back to mock XAI data');
            this.xaiData = this.generateMockXAIData();
            this.updateXAICharts();
        }
    }
    ...
    updateXAICharts() {
        console.log('XAI chart update started');
        // Update XAI related charts
        ...
        console.log('XAI chart update completed');
    }
    ...
    renderFeatureImportance() {
        console.log('Rendering: Feature Importance');
        const container = document.getElementById('feature-importance-chart');
        if (!container) {
            console.warn('Could not find feature-importance-chart container.');
            return;
        }

        const importanceMethods = this.xaiData?.explainability?.feature_importance_methods;
        if (!importanceMethods) {
            container.innerHTML = '<div class="xai-error"><p>Feature importance data not found.</p></div>';
            return;
        }
        ...
        if (rfBuiltin && Array.isArray(rfBuiltin.features) && Array.isArray(rfBuiltin.importance) && rfBuiltin.features.length > 0) {
            ...
            container.innerHTML = '<div class="xai-error"><p>Feature importance data format is incorrect.</p></div>';
        }
    }

    renderSHAPSummaryPlot() {
        console.log('Rendering: SHAP Summary Plot');
        const container = document.getElementById('shap-summary-plot');
        if (!container) {
            console.warn('Could not find shap-summary-plot container.');
            return;
        }
        ...
        if (!shapExplanations || !shapExplanations.random_forest_shap) {
            container.innerHTML = '<div class="xai-error"><p>SHAP Summary Plot data not found.</p></div>';
            return;
        }
        ...
        container.innerHTML = html;
        console.log('SHAP Summary Plot rendering complete');
